# MMRM Descriptive Statistics Table (adapted from mmrm_descriptive)
# Uses random.cdisc.data
...
# Generic treatment handling - uses actual treatment names from data
gt_code <- '
# data is ADQS with ADSL columns already merged

# Get treatment variable and create treatment groups
...
# Add Total rows
total_data <- analysis_data |>
  dplyr::mutate(TRT = "Total")

all_data <- dplyr::bind_rows(analysis_data, total_data)

# Get treatment levels (excluding Total for ordering)
...
baseline_stats <- all_data |>
  ...
  dplyr::summarise(
    ...
    q1 = stats::quantile(AVAL, 0.25, na.rm = TRUE),
    q3 = stats::quantile(AVAL, 0.75, na.rm = TRUE),
...
change_stats <- post_data |>
  ...
  dplyr::summarise(
    ...
    sd = stats::sd(CHG, na.rm = TRUE),
    median = stats::median(CHG, na.rm = TRUE),
    q1 = stats::quantile(CHG, 0.25, na.rm = TRUE),
    q3 = stats::quantile(CHG, 0.75, na.rm = TRUE),
    min = min(CHG, na.rm = TRUE),
    max = max(CHG, na.rm = TRUE),
    .groups = "drop"
  ) |>
  dplyr::mutate(section = "Change from Baseline")

# Combine all stats
all_stats <- dplyr::bind_rows(baseline_stats, value_stats, change_stats) |>
  dplyr::mutate(
    visit_label = dplyr::case_when(
      !is.na(visit_label) ~ visit_label,
      TRUE ~ paste("Week", AVISITN)
    )
  )
...
stats_long <- formatted |>
  dplyr::select(AVISITN, visit_label, section, TRT,
...
# Get population counts for header
pop_counts <- all_data |>
  dplyr::filter(!duplicated(USUBJID)) |>
  dplyr::group_by(TRT) |>
...
gt_tbl <- stats_wide |>
  ...
  gt::tab_header(
    ...
    subtitle = "Actual Value and Change from Baseline by Visit - Summary"
  ) |>
...
run_app(
  blocks = c(
    ...
    gt_table = new_llm_gt_block(
      messages = "Create descriptive statistics table",
      code = gt_code
    )
  ),
  links = c(
