# MMRM Biomarker Interaction Analysis (adapted from mmrm_biomarker)
...
# Adapted from mmrm_biomarker function
# Uses BMRKR2 as biomarker (categorical: LOW, MEDIUM, HIGH)
gt_code <- '
...
# Fit MMRM with full biomarker interaction
fit <- mmrm::mmrm(
...
results |>
  ...
  gt::tab_header(
    title = "Table 14.2.4.2.1",
    subtitle = paste("Change from Baseline at Week", last_visit, "by Biomarker Status - MMRM Analysis")
  ) |>
  gt::tab_source_note(
    source_note = "Model: CHG = TRTN + AVISITN + TRTN*AVISITN + BASE + BMRKR2 + BMRKR2*TRTN + BMRKR2*AVISITN + BMRKR2*TRTN*AVISITN. Unstructured covariance, Kenward-Roger DF."
  ) |>
...
run_app(
  blocks = c(
    ...
    gt_table = new_llm_gt_block(
      messages = "Run MMRM biomarker interaction analysis",
      code = gt_code
    )
  ),
  links = c(
