        ...
        function(input, output, session) {
          ...
          list(
            expr = shiny::reactive({
              ...
              # We need to inject the function and call it with data + params
              fn_text <- r_fn_text()

              if (length(params) == 0) {
                # No params, just call fn(data)
                bquote(
                  {
            ...
            state = list(
              fn = shiny::reactive({
                # Store function as text for serialization
                r_fn_text()
              })
            )
          )
